Profile: KRCore_Observation_VitalSigns_BodyTemperature
...
Description: "KR Core의 활력징후 및 신체 계측 중, 체온 측정 정보에 적용되는 Observation 구조."
...
* code.coding[Loinc] ^short = "체온 측정"
* code.coding[Loinc].code = #8310-5 (exactly)
* code.coding[Loinc].display ^example.label = "Value"
* code.coding[Loinc].display ^example.valueString = "Body temperature"
* value[x] only Quantity
* value[x] ^short = "체온 측정 결과"
* value[x].value 1..
* value[x].value ^short = "측정값"
* value[x].unit ^example.label = "Value"
* value[x].unit ^example.valueString = "°C"
* value[x].system 1..
